...
const featuredPages = [
  {
    ...
    accent: "from-[#7b3dff]/30 to-[#4f00ca]/20",
    icon: (
  ...
  },
  {
    ...
    accent: "from-[#9b6dff]/30 to-[#4f00ca]/20",
    icon: (
      <svg className="h-6 w-6" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth={1.5}>
  ...
  return (
    <div className="overflow-hidden bg-ink text-white">
      ...
      <section className="relative min-h-[92vh] flex flex-col justify-center px-4 py-24">
        ...
        />
        <div
          className="pointer-events-none absolute left-1/2 top-1/2 h-64 w-64 -translate-x-1/2 -translate-y-1/2 rounded-full bg-[#4f00ca]/20 blur-[90px]"
          aria-hidden
        />

        <div className="relative z-10 mx-auto w-full max-w-6xl text-center">
          <span className="mb-6 inline-flex items-center gap-2 rounded-full border border-white/20 bg-white/10 px-4 py-1.5 text-sm font-medium text-brand backdrop-blur-md">
            <span className="relative flex h-2 w-2">
              <span className="absolute inline-flex h-full w-full animate-ping rounded-full bg-[#7b3dff] opacity-75" />
              <span className="relative inline-flex h-2 w-2 rounded-full bg-[#4f00ca]" />
            </span>
          ...
          <div className="mt-10 flex flex-col items-center justify-center gap-4 sm:flex-row">
            <Link
              href="/dating/best-dating-sites-germany"
              className="group inline-flex items-center gap-2 rounded-full bg-gradient-to-r from-[#4f00ca] to-[#7b3dff] px-8 py-4 font-semibold text-white shadow-lg shadow-[0_8px_24px_rgba(79,0,202,0.3)] transition hover:scale-[1.02] hover:shadow-[0_8px_24px_rgba(79,0,202,0.4)]"
            >
              Explore top picks
          ...
          <div className="grid gap-6 md:grid-cols-2 lg:grid-cols-4">
            {featuredPages.map((page) => (
              <Link key={page.href} href={page.href} className="group block">
                <GlassCard className="relative h-full overflow-hidden p-6 transition duration-300 hover:border-[#4f00ca]/40 hover:bg-white/15 hover:shadow-[0_12px_40px_rgba(79,0,202,0.15)]">
                  <div
                  ...
                  <div className="relative">
                    <div className="mb-4 inline-flex rounded-xl border border-white/20 bg-white/10 p-3 text-brand backdrop-blur-sm">
                      {page.icon}
                    </div>
            ...
            {categories.map((category, i) => (
              <Link key={category.href} href={category.href} className="group block">
                <div className="flex h-full flex-col rounded-2xl border border-white/15 bg-gradient-to-br from-white/[0.08] to-white/[0.02] p-6 backdrop-blur-xl transition hover:border-white/30 hover:from-white/[0.12]">
                  <span className="mb-4 flex h-10 w-10 items-center justify-center rounded-full bg-gradient-to-br from-[#4f00ca]/40 to-[#7b3dff]/40 text-sm font-bold text-white">
                    {String(i + 1).padStart(2, "0")}
                  </span>
                  <h3 className="text-lg font-semibold text-white">{category.title}</h3>
                  <p className="mt-2 flex-1 text-sm text-slate-400">{category.description}</p>
                  <span className="mt-4 text-sm font-medium text-brand transition group-hover:text-brand">
                    View category →
                  </span>
                </div>
              </Link>
      ...
      <section className="px-4 py-24">
        <div className="mx-auto max-w-5xl">
          <GlassCard className="border-white/20 bg-white/[0.06] p-8 md:p-12">
            <div className="mb-2 inline-flex rounded-lg bg-[#4f00ca]/20 px-3 py-1 text-xs font-semibold uppercase tracking-wider text-brand">
              Insights
            </div>
            ...
            {faqs.map((faq, i) => (
              ...
              >
                <div className="flex gap-4">
                  <span className="flex h-9 w-9 shrink-0 items-center justify-center rounded-full border border-[#4f00ca]/30 bg-[#4f00ca]/10 text-sm font-bold text-brand">
                    {i + 1}
                  </span>
                  <div>
      ...
      <section className="relative px-4 pb-28 pt-8">
        <div className="pointer-events-none absolute inset-x-0 bottom-0 h-64 bg-gradient-to-t from-[#0c0618]/40 to-transparent" aria-hidden />
        <div className="relative z-10 mx-auto max-w-4xl">
          <div className="overflow-hidden rounded-3xl border border-white/20 bg-gradient-to-br from-[#4f00ca]/30 via-[#7b3dff]/20 to-[#3d00a0]/30 p-10 text-center shadow-[0_20px_60px_rgba(79,0,202,0.2)] backdrop-blur-2xl md:p-16">
            <div
              className="pointer-events-none absolute -right-20 -top-20 h-56 w-56 rounded-full bg-[#7b3dff]/20 blur-3xl"
              aria-hidden
            />
            <div
              className="pointer-events-none absolute -bottom-16 -left-16 h-48 w-48 rounded-full bg-[#4f00ca]/20 blur-3xl"
              aria-hidden
            />
            <div className="relative">
              ...
              <p className="mx-auto mt-5 max-w-xl text-lg text-brand/90">
